...
const sortByChildren = (charArray) => {
    charArray.sort((c1, c2) => {
        const temp = c1.children.length - c2.children.length
        if (temp < 0) {
            return -1;
        }
        if (temp > 0) {
...
const updateNumbers = (obj) => {
    ...
    Object.entries(obj).forEach((element) => {
        tempArr.push(element[0] + ': ' + element[1]);
    });
    return tempArr;
};
...
const houseSize = (arr) => {
    const sizes = [];
    arr.forEach((character) => {
        let m = character.children.length + 1;
        if (character.spouse !== null) {
            m++;
        }
        sizes.push({ house: character.house, members: m });
    });
    return sizes;
};
