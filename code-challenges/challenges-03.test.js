...
const alphabetizeBetter = (arr) => {
    return arr.sort((str1, str2) => str1.toUpperCase() < str2.toUpperCase() ? -1 : 1);
};
...
const sortPeople = (arr) => {
    return arr.sort((p1, p2) => p1.lastName.toUpperCase() < p2.lastName.toUpperCase() ? -1 : 1);
};
...
const sortMeetingsByDay = (arr) => {
    ...
    return arr.sort((m1, m2) => daysOfWeek.indexOf(m1.dayOfWeek) - daysOfWeek.indexOf(m2.dayOfWeek));
};
...
const sortSchedule = (arr) => {
    let temp = arr.sort((m1, m2) => {
        ...
        if (parseInt(m1.start) === parseInt(m2.start)) {
            return parseInt(m1.end) - parseInt(m2.end)
        }

    });
    return sortMeetingsByDay(temp);
};
