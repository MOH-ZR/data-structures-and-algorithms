...
const getNames = (arr) => {
    return arr.map((person) => {
        return person.name.split('').reduce((reversed, letter) => {
...
const count = (target, input) => {
    return input.reduce((counter, innerArr) => {
...
const totalSum = (input) => {
    return input.reduce((sum, innerArr) => {
    ...
    input.forEach((arr) => {
        ...
        arr.forEach((element) => {
            if (typeof element === 'number' && element % 5 === 0) {
                innerArr.push(Math.pow(2, element));
            }
        });
...
let findMaleAndFemale = (data) => {
    let starNames;
    starNames = data.reduce((names, star) => {
        if (star.gender === 'male' || star.gender === 'female') {
            names += star.name + ' and ';
        }
        return names;
    }, '');
...
let findShortest = (data) => {
    let ordered = data.sort((c1, c2) => {
        return parseInt(c1.height) - parseInt(c2.height);
    });
