...
const addPurchases = (arr) => {
    return arr.reduce((totalPrice, obj) => {
        totalPrice += obj.purchasePrice;
        return totalPrice;
    }, 0);
};
...
const returnNames = (arr) => {
    return arr.reduce((names, starWar) => {
        names.push(starWar.name);
        return names;
    }, []);
};
...
const reversedString = (str) => {
    ...
    return str.split('').reduce((reversedStr, letter) => {
...
const calculateAverage = (arr) => {
    let result = {};
    result = arr.reduce((accumulator, value) => {
        accumulator.count++;
...
const extractChildren = (arr) => {
    ...
    }).reduce((names, character) => {
        if ('children' in character) {
            character.children.forEach((child) => names.push(child));
        }
        return names;
    }, []);
};
